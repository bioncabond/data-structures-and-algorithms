...
const addValues = (arr) => {
  // Solution code here...
  let sumNumsArray = arr.reduce((acc, num) => {
    return acc + num;
  }, 0);
...
const addPurchases = (arr) => {
  ...
  let totalPurchases = arr.reduce((acc, idx)=> {
    acc  =  acc + idx.purchasePrice;
    return acc;
  }, 0)
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let countArray = arr.reduce((acc,num,idx)=>{
    return idx + 1; 
  }, 0)
    return countArray;
};
