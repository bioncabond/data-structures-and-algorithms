...
const count = (target, input) => {
  ...
  let num = 0;
  input.map((arr) => {
    num += arr.filter((value) => (value === target)).length;
  });
  return num;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, value) => acc + value.reduce(
    (inneracc, newval) => inneracc + newval, 0), 0);
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortie, nextCharacter) =>
    Number(shortie.height) < Number(nextCharacter.height) ?
      shortie : nextCharacter).name;
};
